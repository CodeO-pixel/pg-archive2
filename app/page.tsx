...
import { 
  Upload, User, Eye, ArrowLeft, Download, Trash2, Clock, Sparkles, UserPlus, Shield, Search, Tag, Filter, Home as HomeIcon, BookOpen, ChevronLeft, ChevronRight, Bell, Heart, Maximize, Minimize, Rows, FileText, BookMarked
} from 'lucide-react';
...
export default function Home() {
  ...
  const [readingMode, setReadingMode] = useState<'cascada' | 'comic' | 'manga'>('cascada');
  ...
  const [userAdminTab, setUserAdminTab] = useState<'buscar' | 'crear'>('buscar');
  const [userSearchQuery, setUserSearchQuery] = useState('');
  const [userSearchResults, setUserSearchResults] = useState<{ id: string; username: string; email: string; role: string }[]>([]);
  ...
  useEffect(() => {
    ...
    // Restaurar el modo de lectura preferido guardado en este navegador
    if (typeof window !== 'undefined') {
      const savedMode = localStorage.getItem('pgarchive_reading_mode');
      if (savedMode === 'cascada' || savedMode === 'comic' || savedMode === 'manga') {
        setReadingMode(savedMode);
      }
      try {
        const savedRead = localStorage.getItem('pgarchive_read_chapters');
        if (savedRead) setReadChapterIds(new Set(JSON.parse(savedRead)));
      } catch {
        // ignorar si el dato guardado está corrupto
      }
    }
  ...
  // Recordar qué obra/capítulo se está leyendo, para retomarlo si se refresca la página
  useEffect(() => {
    if (typeof window === 'undefined') return;
    if (selectedSeries) {
      localStorage.setItem('pgarchive_last_read', JSON.stringify({
        seriesId: selectedSeries.id,
        chapterId: selectedChapter ? selectedChapter.id : null,
      }));
    } else {
      localStorage.removeItem('pgarchive_last_read');
    }
  }, [selectedSeries?.id, selectedChapter?.id]);

  // Al abrir un capítulo, retomar en qué página se había quedado
  useEffect(() => {
    if (typeof window === 'undefined' || !selectedChapter) return;
    const saved = localStorage.getItem(`pgarchive_page_${selectedChapter.id}`);
    const idx = saved ? parseInt(saved, 10) : 0;
    setCurrentPageIndex(Number.isFinite(idx) ? idx : 0);
    pageRefs.current = [];
  }, [selectedChapter?.id]);

  // Guardar la página actual cada vez que cambia (funciona igual en cascada, cómic y manga)
  useEffect(() => {
    if (typeof window === 'undefined' || !selectedChapter) return;
    localStorage.setItem(`pgarchive_page_${selectedChapter.id}`, String(currentPageIndex));
  }, [currentPageIndex, selectedChapter?.id]);

  // Marcar el capítulo como leído al llegar a la última página
  useEffect(() => {
    if (!selectedChapter) return;
    const totalPages = getPageUrls(selectedChapter.pages).length;
    if (totalPages > 0 && currentPageIndex >= totalPages - 1 && !readChapterIds.has(selectedChapter.id)) {
      setReadChapterIds(prev => {
        const next = new Set(prev).add(selectedChapter.id);
        if (typeof window !== 'undefined') {
          localStorage.setItem('pgarchive_read_chapters', JSON.stringify(Array.from(next)));
        }
        return next;
      });
    }
  ...
  // Recordar el modo de lectura preferido en este navegador
  useEffect(() => {
    if (typeof window === 'undefined') return;
    localStorage.setItem('pgarchive_reading_mode', readingMode);
  }, [readingMode]);

  // En modo cascada: detectar qué página está en pantalla mientras se hace scroll
  useEffect(() => {
    if (readingMode !== 'cascada' || !selectedChapter) return;
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        if (best) {
          ...
          if (!isNaN(idx)) setCurrentPageIndex(idx);
        }
      },
      { threshold: [0.4, 0.6] }
    );
  ...
  }, [readingMode, selectedChapter?.id]);

  // Al abrir un capítulo en modo cascada, hacer scroll hasta la página guardada
  useEffect(() => {
    if (readingMode !== 'cascada' || !selectedChapter) return;
    const saved = localStorage.getItem(`pgarchive_page_${selectedChapter.id}`);
    const idx = saved ? parseInt(saved, 10) : 0;
    if (idx > 0) {
      const timer = setTimeout(() => {
        pageRefs.current[idx]?.scrollIntoView({ block: 'start' });
      }, 200);
      return () => clearTimeout(timer);
    }
  }, [selectedChapter?.id, readingMode]);

  // Navegación por teclado en modo cómic/manga (flechas del teclado)
  useEffect(() => {
    if (readingMode === 'cascada' || !selectedChapter) return;
    const totalPages = getPageUrls(selectedChapter.pages).length;
    const handleKeyDown = (e: KeyboardEvent) => {
      const forwardKey = readingMode === 'manga' ? 'ArrowLeft' : 'ArrowRight';
      const backKey = readingMode === 'manga' ? 'ArrowRight' : 'ArrowLeft';
      if (e.key === forwardKey) setCurrentPageIndex(p => Math.min(p + 1, totalPages - 1));
      if (e.key === backKey) setCurrentPageIndex(p => Math.max(p - 1, 0));
    };
    window.addEventListener('keydown', handleKeyDown);
    return () => window.removeEventListener('keydown', handleKeyDown);
  }, [readingMode, selectedChapter?.id, selectedChapter?.pages]);

  // Detectar cuándo se entra/sale de pantalla completa (incluye cuando se sale con ESC)
  useEffect(() => {
  ...
  const toggleFullscreen = () => {
    if (!document.fullscreenElement) {
      readerRef.current?.requestFullscreen().catch(() => {
        alert('Tu navegador no permite pantalla completa aquí.');
      });
    } else {
  ...
  const fetchData = async () => {
    try {
      ...
      // Retomar donde te habías quedado leyendo antes de refrescar la página
      if (typeof window !== 'undefined' && seriesData && chaptersData) {
        try {
          const raw = localStorage.getItem('pgarchive_last_read');
          if (raw) {
            const { seriesId, chapterId } = JSON.parse(raw);
            const series = seriesData.find((s: Series) => s.id === seriesId);
            const chapter = chapterId ? chaptersData.find((c: Chapter) => c.id === chapterId) : null;
            if (series) {
          ...
          // localStorage corrupto o inaccesible: no pasa nada, simplemente no se restaura
        }
      }
    } catch (err) {
  ...
  const handleSearchUsers = async (e?: React.FormEvent) => {
    ...
    try {
      const { data, error } = await supabase.rpc('admin_search_users', { search_query: userSearchQuery.trim() });
      if (error) throw error;
      setUserSearchResults(data || []);
    } catch (err: any) {
      alert('Error buscando usuarios: ' + err.message);
    } finally {
      setSearchingUsers(false);
    }
  };

  const handleChangeUserRole = async (userId: string, newRole: 'admin' | 'scan' | 'lector') => {
    try {
      const { error } = await supabase.rpc('admin_update_user_role', { target_user_id: userId, new_role: newRole });
      if (error) throw error;
      setUserSearchResults(prev => prev.map(u => (u.id === userId ? { ...u, role: newRole } : u)));
    } catch (err: any) {
      alert('Error cambiando el rol: ' + err.message);
    }
  };
      ...
      <main className="flex-1 p-4 sm:p-6 max-w-6xl mx-auto w-full safe-bottom">
        {selectedChapter ? (
          ...
          <div>
            ...
            {/* MODO DE LECTURA + PANTALLA COMPLETA */}
            <div className="flex flex-wrap justify-between items-center gap-3 mb-6 max-w-3xl mx-auto">
              <div className="flex gap-1 bg-slate-950 p-1 rounded-lg border border-slate-800">
                <button
                  onClick={() => setReadingMode('cascada')}
                  className={`px-3 py-1.5 text-xs font-bold rounded-md flex items-center gap-1.5 transition ${readingMode === 'cascada' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  title="Cascada: todas las páginas seguidas (ideal para manhwa/manhua)"
                >
                  <Rows className="w-3.5 h-3.5" /> Cascada
                </button>
                <button
                  onClick={() => setReadingMode('comic')}
                  className={`px-3 py-1.5 text-xs font-bold rounded-md flex items-center gap-1.5 transition ${readingMode === 'comic' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  title="Cómic: una página a la vez, de izquierda a derecha"
                >
                  <FileText className="w-3.5 h-3.5" /> Cómic
                </button>
                <button
                  onClick={() => setReadingMode('manga')}
                  className={`px-3 py-1.5 text-xs font-bold rounded-md flex items-center gap-1.5 transition ${readingMode === 'manga' ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-white'}`}
                  title="Manga: una página a la vez, de derecha a izquierda"
                >
                  <BookMarked className="w-3.5 h-3.5" /> Manga
                </button>
              </div>
              <button
                onClick={toggleFullscreen}
                className="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-300 p-2 rounded-lg transition"
            ...
            {readingMode !== 'cascada' && (
              <div className="flex justify-between items-center gap-3 mb-4 max-w-3xl mx-auto">
                <button
                  onClick={() => setCurrentPageIndex(p => Math.max(p - 1, 0))}
                  disabled={readingMode === 'manga' ? currentPageIndex >= getPageUrls(selectedChapter.pages).length - 1 : currentPageIndex <= 0}
                  className="bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-600 text-cyan-400 px-4 py-2 rounded-lg font-bold text-sm transition"
                >
                  {readingMode === 'manga' ? '❮ Siguiente' : '❮ Anterior'}
                </button>
                <select
                  value={currentPageIndex}
                  onChange={e => setCurrentPageIndex(Number(e.target.value))}
                  className="bg-slate-800 border border-slate-600 text-white font-bold text-xs px-3 py-2.5 rounded-lg focus:outline-none focus:border-cyan-500 text-center"
                  title="Ir a una página específica"
                >
                  {getPageUrls(selectedChapter.pages).map((_, idx) => (
                    <option key={idx} value={idx}>
                      Página {idx + 1} de {getPageUrls(selectedChapter.pages).length}
                    </option>
                  ))}
                </select>
                <button
                  onClick={() => setCurrentPageIndex(p => Math.min(p + 1, getPageUrls(selectedChapter.pages).length - 1))}
                  disabled={readingMode === 'manga' ? currentPageIndex <= 0 : currentPageIndex >= getPageUrls(selectedChapter.pages).length - 1}
                  className="bg-slate-800 hover:bg-slate-700 disabled:opacity-40 border border-slate-600 text-cyan-400 px-4 py-2 rounded-lg font-bold text-sm transition"
                >
                  {readingMode === 'manga' ? 'Anterior ❯' : 'Siguiente ❯'}
                </button>
              </div>
            )}

            <div ref={readerRef} className="bg-slate-950">
              {readingMode === 'cascada' ? (
                ...
                <div className="flex items-center justify-center max-w-3xl mx-auto">
                  <img
                    src={getPageUrls(selectedChapter.pages)[currentPageIndex]}
                    alt={`Página ${currentPageIndex + 1}`}
                    className="max-w-full max-h-[85vh] mx-auto rounded-lg shadow-2xl border border-slate-800"
                  />
                </div>
              )}

              {isFullscreen && (
                <div className="fixed bottom-4 left-1/2 -translate-x-1/2 bg-slate-900/90 border border-slate-700 rounded-full px-4 py-2 flex items-center gap-3 shadow-2xl z-[60]">
                  {readingMode !== 'cascada' && (
                    <span className="text-xs font-bold text-white">{currentPageIndex + 1} / {getPageUrls(selectedChapter.pages).length}</span>
                  )}
                  <button onClick={toggleFullscreen} className="text-xs font-bold text-cyan-400 flex items-center gap-1">
          ...
          <div>
                ...
                <div className="flex items-center gap-2">
                  <button 
                    onClick={() => {
                      ...
                      setTargetSeriesId(selectedSeries.id);
                      setShowUpload(true);
                    }}
                    className="bg-emerald-950 hover:bg-emerald-900 border border-emerald-700 text-emerald-300 px-4 py-2 rounded-lg font-bold flex items-center space-x-2 text-sm transition"
                  >
                    <Upload className="w-4 h-4" /> <span>Subir Capítulos</span>
                  </button>
                  <button 
            ...
            <div className="space-y-3">
              {chaptersList.filter(c => c.series_id === selectedSeries.id).map(chap => {
                ...
                return (
                <div key={chap.id} className={`border p-4 rounded-xl flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 transition shadow ${isRead ? 'bg-slate-900/40 border-slate-800/60 opacity-60 hover:opacity-100' : 'bg-slate-900 border-slate-800 hover:border-cyan-500/50'}`}>
                  <div className="min-w-0">
                    <h4 className="font-bold text-white text-base sm:text-lg break-words flex items-center gap-2">
                      {chap.title}
                      {isRead && (
                        <span className="text-[10px] font-bold text-emerald-400 border border-emerald-800 bg-emerald-950 px-1.5 py-0.5 rounded uppercase tracking-wide shrink-0">
                          ✓ Leído
                        </span>
                      )}
                    </h4>
                  ...
                  <div className="flex items-center space-x-2 shrink-0">
                    ...
                    >
                      <Eye className="w-4 h-4" /> <span>{isRead ? 'Releer' : 'Leer'}</span>
                    </button>
                  </div>
                </div>
                );
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-slate-900 border border-purple-800 rounded-xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="flex gap-2 mb-4 bg-slate-950 p-1 rounded-lg border border-slate-800">
              <button
                type="button"
                onClick={() => setUserAdminTab('buscar')}
                className={`flex-1 py-2 text-xs font-bold rounded-md transition ${userAdminTab === 'buscar' ? 'bg-purple-600 text-white' : 'text-slate-400 hover:text-white'}`}
              >
                Buscar y Cambiar Rol
              </button>
              <button
                type="button"
                onClick={() => setUserAdminTab('crear')}
                className={`flex-1 py-2 text-xs font-bold rounded-md transition ${userAdminTab === 'crear' ? 'bg-purple-600 text-white' : 'text-slate-400 hover:text-white'}`}
              >
                Crear Cuenta Nueva
              </button>
            </div>
              ...
              <div className="space-y-3">
                <form onSubmit={handleSearchUsers} className="flex gap-2">
                  <input
                    ...
                    className="flex-1 bg-slate-950 border border-slate-800 rounded-lg p-3 text-sm text-white focus:outline-none focus:border-purple-500"
                  />
                  <button type="submit" disabled={searchingUsers} className="px-4 bg-purple-600 hover:bg-purple-500 text-white font-bold rounded-lg text-xs border border-purple-400">
                    <Search className="w-4 h-4" />
                  </button>
                </form>

                {searchingUsers ? (
                  <p className="text-xs text-slate-500 text-center py-4">Buscando...</p>
                ) : userSearchResults.length === 0 ? (
                  <p className="text-xs text-slate-500 text-center py-4">
                    {userSearchQuery ? 'Sin resultados.' : 'Escribe un nombre de usuario para buscar.'}
                  </p>
                ) : (
                  <div className="space-y-2 max-h-72 overflow-y-auto">
                    {userSearchResults.map(u => (
                      <div key={u.id} className="bg-slate-950 border border-slate-800 rounded-lg p-3 flex items-center justify-between gap-2">
                        <div className="min-w-0">
                          <p className="text-sm font-bold text-white truncate">{u.username || '(sin usuario)'}</p>
                          <p className="text-[11px] text-slate-500 truncate">{u.email}</p>
                        </div>
                        <select
                          value={u.role}
                          onChange={e => handleChangeUserRole(u.id, e.target.value as 'admin' | 'scan' | 'lector')}
                          className="bg-slate-900 border border-slate-700 text-white text-xs font-bold p-2 rounded-lg focus:outline-none focus:border-purple-500 shrink-0"
                        >
